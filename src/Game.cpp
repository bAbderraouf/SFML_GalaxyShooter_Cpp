    ...
    void Game::initEnemy()
    {
       ...
       */
        ...
        this->enemie_texture.loadFromFile("assets/images/fight_plane_1.png");
...
*/
        // player as a plane:
        this->player_texture.loadFromFile("assets/images/player.png");
    ...
    void Game::updateEnemies()
    {
       ...
       if( sf::Mouse::isButtonPressed(sf::Mouse::Left)  )
       {
            ...
            if( this->mouseClicked == false )
            {
                ...
                for(auto itee = enemies.begin() ; itee != enemies.end() && deleted_flag==false ;  )
                {
                        // check if clicked on the enemy then rsemove it       
                        if( itee->getGlobalBounds().contains(this->mousePosView) ) 
                        {
                            deleted_flag = true;
    ...
    void Game::updateShoots()
    {
        ...
        */
        ...
        float pas = 15.f;
        bool shootErased = false, enemieErased = false;;
        bool yCondition = false , xCondition = false , 
            yplayerColisionCondition = false , xplayerColisionCondition =false;
        
        for(auto its = shoots.begin() ; its != shoots.end(); )
        {
            shootErased = false;

            // dell shoot if its out of screan
            if( its->getPosition().y + pas <= 0)
            {
                its = this->shoots.erase(its);
            ...
            for (auto ite = enemies.begin(); ite != enemies.end(); )
            {
               ...
               // std::cout << "enmy size before : " <<this->enemies.size() << std::endl;
                ...
                yCondition = ( its->getPosition().y <=  ite->getPosition().y + (ite->getTexture()->getSize().y * ite->getScale().y)  && 
                                its->getPosition().y >=  ite->getPosition().y );
                xCondition = ( its->getPosition().x >= ite->getPosition().x  &&  
                                its->getPosition().x <=  ite->getPosition().x +  (ite->getTexture()->getSize().x * ite->getScale().x) );
                
                yplayerColisionCondition = ( this->player.getPosition().y <=  ite->getPosition().y + (ite->getTexture()->getSize().y * ite->getScale().y)  );
                xplayerColisionCondition = ( this->player.getPosition().x + (this->player.getTexture()->getSize().x * player.getScale().x) >=  ite->getPosition().x  && 
                                             this->player.getPosition().x <  ite->getPosition().x + (ite->getTexture()->getSize().x * ite->getScale().x) );
                ...
                if(enemieErased == false)
                {
                    ite++;
                }
                else
                {
                    break;  // enemies loop (got to next shoot)

                }

            }

            if ( shootErased == false)
            {
                its++;
            }
    ...
    void Game::calculateScore(const sf::Sprite& sp)
    {
        /*
            update score value,
        ...
        */
        ...
        float enemyScale = sp.getScale().x;
